...
export class AuxPoW {
  ...
  parent_coinbase_tx: {
    ...
    txin_count: number
    txin: Array<{
      prev_out_point_hash: string
      prev_out_point_index: string
      sig_script_len: number
      sig_script: string
      sequence: number
    }>
    txout_count: number
  ...
  parent_coinbase_merkle_count: number
  parent_coinbase_merkle: Array<string>
  parent_coinbase_merkle_bitmask: number
  ...
  processBuffer() {
    ...
    for (let i = 0; i < this.parent_coinbase_tx.txin_count; i++) {
      let txinObj: any = {}
    ...
    for (let i = 0; i < this.parent_coinbase_tx.txout_count; i++) {
      let txoutObj: any = {}
    ...
    const btcHeaderStartPtr = this.bufPtr
  ...
  ************************************************************************
  ...
  readBufHex(numBits: number, littleEndian = true) {
    const newBuf = Buffer.alloc(numBits / 8)
